...
(function(){
  ...
  theme.pageTransitions = function() {
    if (document.body.dataset.transitions === 'true') {
      ...
      /** Code that blocking opening link in new window with ctrl+click **/ 
      ...
      //   });
      // });
      /** End Code that blocking opening link in new window with ctrl+click **/ 
...
if (screen.width > 767) {
  $(window).scroll(function(){
        ...
        $('.background-media-text').each(function(){
          ...
          $(this).find('.background-media-text__image').css('background-position-y', half);
          } else {

        $(this).find('.background-media-text__image').css('background-position-y', '0');
          }
        });
...
//     $('.ProductForm__AddToCart_custom').on('click', function(){
//       $(this).find('span').text('Added to Cart');
//        setTimeout(function(){
//            var is_cart_trigger = 'true';
// //            cartUpdateJs(is_cart_trigger); 
//          },1200);   
//     });
    
          $('body').on('click','.custom--drawer .js-qty__wrapper .js-qty__adjust--plus, .custom--drawer .js-qty__wrapper .js-qty__adjust--minus', function(){
            var aria_label = $(this).attr('aria-label');
            var data_id = $(this).parent().find('.js-qty__num').attr('data-id');
            var data_qnt = $(this).parent().find('.js-qty__num').val();
            if(aria_label == 'Increase item quantity by one'){
            var data_qnt = parseInt(data_qnt) + 1;
            }else if(aria_label == 'Reduce item quantity by one'){
            var data_qnt = parseInt(data_qnt) - 1;
            }            
            addItemToCartcustom(data_id,data_qnt);
          });
    
          $('body').on('keyup','.custom--drawer .js-qty__wrapper .js-qty__num', function(){
            var data_id = $(this).attr('data-id');
            var data_qnt = parseInt($(this).val());  
            addItemToCartcustom(data_id,data_qnt);
          });
    
    $('body').on('click', '.addcart-btn .add-to-cart', function(){
     $(this).find('span').text('Added to Cart');
     setTimeout(function(){
       $.getJSON('/cart.json', function(cart) {
        ...
        // var total_price = $('.cart__subtotal div').html();
         $('#StickyItems').html(item_count +' Item');
         $('#StickySubtotal').html('Rs. '+ total_price.toFixed(2));
         $('.sticky-cart').addClass('sticky-cart--open');
         }
       });
    ...
    $('body').on('click','.add-to-cart-btn .ProductForm__AddToCart_custom_recently_view', function(){
     $(this).find('span').text('Added to Cart');
      var variant_id = $(this).attr('data-vid');
     addItemToCartcustomrecently(variant_id,1);
    });     
    
       
     $('body').on('click','.drawer__scrollable #remove_item', function(){
      var variant_item_id = $(this).attr('data_item');
        console.log('tesint_2')
      addItemToCartcustom(variant_item_id,0);
       
    });

   $('body').on('click','#CartPageForm #remove_item', function(){
      var variant_item_id = $(this).attr('data_item');
        console.log('tesint_3')
...
function addItemToCartcustomrecently (variant_id, quantity) {
    var data = {
      "quantity": quantity,
      "id": variant_id,
    }
...
function addItemToCartcustom (variant_id, quantity) {
    var data = {
      "quantity": quantity,
      "id": variant_id,
    }
...
function cartUpdateJs(is_cart_trigger){
  $.ajax({url: "https://gillori.com/cart?t=1656567244882&view=ajax", 
          success: function(result) {  
            ...
            $('.drawer__scrollable').addClass('custom--drawer');
            $('.drawer__scrollable div').html(new_result);
            
            var cart__item = $('.custom--drawer .cart__item').length;
            if(cart__item < 1){
             $('.drawer__inner').css('display','none'); 
             $('.drawer__cart-empty').css('display','block');
            }else{
              $('.drawer__inner').css('display','block'); 
              $('.drawer__cart-empty').css('display','none');
            }
            ...
            $.getJSON('/cart.json', function(cart) {
              ...
              var cart_total_price = parseFloat(cart.total_price) / 100;                  
              $('.cart__item-sub div').next().html('Rs '+ cart_total_price.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,").toString());

            });

            if(is_cart_trigger == 'true'){
  ...
  $('.drawer__close .js-drawer-close').on('click', function(){
    $('.drawer--cart drawer--cart--static drawer--is-open').removeClass(this);
    $('drawer drawer--cart drawer--cart--static').addClass(this);
  });

// Product quantity JS starts here//
...
setTimeout(function(){
$('.quick-view.js-no-transition').attr('href','javascript:void(0)')
  $('.quick-view').attr('href','javascript:void(0)')
},2000)
